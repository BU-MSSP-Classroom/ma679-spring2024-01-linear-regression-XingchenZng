...
lmfit_2008<-lm(log(SalePrice)~log(`Lot Area`)+log(`Overall Qual`)+log(`Gr Liv Area`)+log(`Garage Area`+1),ames_raw_2008)
summary(lmfit_2008)
```
...
Your code:
```{r,fig.width=9,fig.height=7,echo=show_code}
residuals <- resid(lmfit_2008)
plot(predict(lmfit_2008), residuals)
par(mfrow = c(2, 2))
plot(lmfit_2008)
...
Yes. As all the p-values are so small that they are all statistically significant.
...
Your code:
```{r,echo=TRUE}
lmfit_interaction <- lm(log(SalePrice)~log(`Lot Area`)*log(`Overall Qual`)+log(`Gr Liv Area`)*log(`Garage Area`+1), data = ames_raw_2008)
summary(lmfit_interaction)
...
~~~
From the result, we can see that "Lot Area" and "Overall Qual" has interaction due to the big p-value.
...
Your code:
```{r,echo=TRUE}
lm_poly1 <- lm(log(SalePrice) ~ poly(log(`Lot Area`), 3), data = ames_raw_2008)
lm_poly2 <- lm(log(SalePrice) ~ poly(log(`Overall Qual`), 3), data = ames_raw_2008)
lm_poly3 <- lm(log(SalePrice) ~ poly(log(`Gr Liv Area`), 3), data = ames_raw_2008)
summary(lm_poly1)
summary(lm_poly2)
summary(lm_poly3)
```
...
Your code:
```{r,echo=TRUE}
lmfit_2008<- lm(log(SalePrice)~`Lot Area`+`Overall Qual`+`Gr Liv Area`+`Garage Area`,ames_raw_2008)
...
There is a relationship between the predictor and the response, since the p-value associated with the predictor is statistically significant.

ii. How strong is the relationship between the predictor and the response?
The relationship is moderately strong, indicated by the R-squared value, which measures the proportion of variance in the response explained by the predictor.

iii. Is the relationship between the predictor and the response positive or negative?
It is negative.

iv. What is the predicted mpg associated with a horsepower of 98? What are the associated 95% confidence and prediction intervals?
The predicted mpg is 24.4671. When it comes to 95% confidence interval, the predicted mpg is 24.9406, the prediction interval is from 15.2825 to 34.5987.
...
i. Is there a relationship between the predictors and the response?
  Yes, by looking at the F-statistic, there is at least 1 predictor is related to the response.
  
ii. Which predictors appear to have a statistically significant relationship to the response?
  Displacement, weight, year and origin.
  
iii. What does the coefficient for the year variable suggest?
  If there is 1 unit increase of year, there will be 0.7508 increase in the average of mpg.
...
Your code:
```{r,echo=TRUE}
interaction_model <- lm(mpg ~ horsepower * weight, data = Auto)
...
~~~
I add the interaction term because I would like to check if there is relationship between the two variables. According to the results, there is small p-value, indicates that all the coefficients are statistically significant. As a result, we should keep all the coefficient.
...
Your code:
```{r,echo=TRUE}
model_original <- lm(Sales ~ Price + Urban + US, data = Carseats)
summary(model_original)
model_log_price <- lm(Sales ~ log(Price) + Urban + US, data = Carseats)
model_sqrt_price <- lm(Sales ~ sqrt(Price) + Urban + US, data = Carseats)
model_price_squared <- lm(Sales ~ I(Price^2) + Urban + US, data = Carseats)
summary(model_log_price)
summary(model_sqrt_price)
summary(model_price_squared)
...
~~~
The intercept represents the expected value of Sales is 13/0435 when all other predictors are zero. 
The coefficient for Price represents the expected change in Sales is 0.0545 for a one-unit increase in Price, holding Urban and US constant. 
UrbanYes represents the expected difference in Sales is 0.0219 between an Urban location (Urban = Yes) and a non-Urban location (Urban = No), holding Price and US constant.
USYes represents the expected difference in Sales is 1.2006 between a product sold in the US (US = Yes) and a product sold outside the US (US = No), holding Price and Urban constant.
~~~
...
~~~
$Sales$=$\beta Price$$*$Price+$\beta UrbanYes$$*$UrbanYes+$\beta USYes*$USYes
...
conf_intervals
# Create diagnostic plots for the smaller model
...
~~~
There are several outliers, but only a few.
